  ...
  if (sortquery === "desc") {
    ...
    if (getProb.length !== 0) {
      if (limited > parseInt(getLimitedQuery)) {
        res
          .status(200)
          .json({ getProb, hasMore: false, totalLength: getLimitedQuery });
      } else {
        res
          .status(200)
          .json({ getProb, hasMore: true, totalLength: getLimitedQuery });
      }
    } else {
      res.status(200).json("Not Found");
    }
...
router.get("/userFinished", async (req, res) => {
  const userId = req.user.id;
  try {
    const ProbFinshed = await Prob.find({ isPrivate: false, ans: userId });
    res.status(200).json(ProbFinshed);
  } catch (err) {
